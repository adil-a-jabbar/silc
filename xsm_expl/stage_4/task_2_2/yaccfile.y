...
%token NUM PLUS MINUS MUL DIV END SPACE NEWLINE ID ASSG READ WRITE START COLON LT LE GT GE EQ NE IF THEN ELSE ENDIF WHILE ENDWHILE DO BREAK CONT DECL ENDDECL INT STR COMMA STRING_TYPE
...
| NUM {$$=$1;}
| ID {$$=$1; $1->Gentry = setGentry ($1->varName); $1->type = $1->Gentry->type; $1->val=0;}
| ID '[' NUM ']' {$$=$1; $1->Gentry = setGentry ($1->varName); $1->type = $1->Gentry->type; $1->val=$3->val;};
...
VarList:    VarList COMMA ID {install ($3->varName, type, 1); $$->type = type;}
        |   VarList COMMA ID '[' NUM ']' {install ($3->varName, type, $5->val); $$->type = type;}
        |   ID {install ($1->varName, type, 1); $$->type = type;}
        |   ID '[' NUM ']' {install ($1->varName, type, $3->val); $$->type = type;};
