...
Fdef: Type ID '(' ParamList ')' '{' LdeclBlock Body '}' {currGSymbol = setGentry($2->varName); if (type != currGSymbol->type) {yyerror ("invalid type for function\n");} $8->nodeType = FUN_DEF; checkNameEquivalence ($2->varName); FILE* filePointer = fopen ("executable.xsm", "a"); codeGen ($8, filePointer); fclose (filePointer); paramList=NULL; paramListTail=NULL; Lhead=NULL; Ltail=NULL; currPBinding=1; currNBinding=-1;};
...
MainBlock: Type MAIN '(' ')' '{' LdeclBlock Body '}' {$7->nodeType = MAIN_FUN; FILE* filePointer = fopen ("executable.xsm", "a"); codeGen ($7, filePointer); fclose (filePointer); paramList=NULL; paramListTail=NULL; Lhead=NULL; Ltail=NULL; currPBinding=1; currNBinding=-1;};
