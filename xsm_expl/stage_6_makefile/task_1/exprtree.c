...
struct tnode * makeStmtNode (int nodeType, struct tnode * left, struct tnode * right, struct tnode * mid){
    ...
    if (nodeType == OUTPUT_STMT){
        if (left->type != intType && left->type != strType && left->type != nullType) {
            char msg[40];
    ...
    if ((nodeType == ASG_STMT && (right->type != nullType && left->type != right->type)) | (nodeType == ASG_STMT && (left->nodeType != IDENT && left->nodeType != FIELD_NODE) )){
...
int codeGen (struct tnode *t, FILE * filePointer){
    ...
    if (t->nodeType == CONNECTOR){
        ...
        else if (t->left->left == NULL) {//no expr (not array)
        ...
        else if (t->left->nodeType == IDENT) {
            int resultReg = codeGen (t->left->left, filePointer);
            if (t->left->left->nodeType == FIELD_NODE){
                fprintf (filePointer, "MOV R%d, [R%d]\n", resultReg, resultReg);
            }
        ...
        if (t->left->left == NULL  || (t->left->left != NULL && t->left->nodeType == FIELD_NODE) || t->left->type == strType || t->left->nodeType == OP) {//no expr (not array)
        ...
        else if (t->left->left != NULL && t->left->nodeType != FIELD_NODE){  //array[]
        ...
        if ((t->left != NULL) && (t->left->nodeType == IDENT)){
            resultReg = codeGen (t->left, filePointer);
            int temp = FLookup(t->left->type, t->right->varName)->fieldIndex;
            fprintf (filePointer, "ADD R%d, %d\n", resultReg, temp);
            return resultReg;
        }
        return -1;
