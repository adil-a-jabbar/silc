...
%type <no> expr NUM program END Slist Stmt InputStmt OutputStmt AsgStmt IfStmt WhileStmt BreakStmt ContinueStmt FreeStmt ID TypeName String STRING_TYPE TypeDefBlock GdeclBlock FdefBlock MainBlock FieldDeclList FieldDecl TypeDefList GdeclList LdeclBlock GidList Gid IdList Fdef Body Retstmt ArgList INT STR Field ALLOC INITIALIZE FREE NULLVAL Type
%token NUM PLUS MINUS MUL DIV MOD END SPACE NEWLINE ID ASSG READ WRITE START COLON LT LE GT GE EQ NE OR AND IF THEN ELSE ENDIF WHILE ENDWHILE DO BREAK CONT DECL ENDDECL INT STR COMMA STRING_TYPE MAIN RETURN TYPE ENDTYPE ALLOC INITIALIZE FREE NULLVAL
%left AND OR 
